...
define( ["qlik"
	...
	function (qlik, $, props, colorPalette, cssContent, template, d3, localRequire) {//, lasso
	'use strict';

    $("<style>").html(cssContent).appendTo("head");
	var tooltipDateFormat = d3.timeFormat("%b %Y");

	function qlikNumberToDate(num) {
		// Qlik date serial base is 1899-12-30.
		var qlikEpochMs = Date.UTC(1899, 11, 30);
		return new Date(qlikEpochMs + (num * 86400000));
	}
	...
	function parseDateValue(rawValue) {
		...
		if (typeof rawValue === "number" && isFinite(rawValue)) {
			return qlikNumberToDate(rawValue);
		}
		if (typeof rawValue === "string") {
			...
			// Fallback for ambiguous slash dates.
			var match = rawValue.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})$/);
			if (match) {
	...
	function addDays(date, days) {
		...
		return new Date(millis + (days * 86400000));
	}

	function barWidthFromDates(d, minDate, xScale) {
		var durationMs = d.TimeEnd.getTime() - d.TimeStart.getTime();
		var dateAtDuration = new Date(minDate.getTime() + durationMs);
		return xScale(dateAtDuration);
	}
	//////////////////////////////////////////////
		...
		bars.enter()
			...
			.transition()
				...
				.attr('width', function(d){
					return barWidthFromDates(d, minDate, xScale);
				})
				.attr("x", function(d, i) {
...
//replace the three statements below
	...
	function tooltipStart(d, i, bars) {
		...
		tooltipDiv.transition()
			.duration(200)
			.style("opacity", .9);
		tooltipDiv .html( d.Dim+ " from " + tooltipDateFormat(d.TimeStart) + ' to ' + tooltipDateFormat(d.TimeEnd))
	...
	function dataObj(layout) {
		...
		let data = ganttData.map(function (inner_d) {
			...
			var startDateInput = inner_d.dim_1_num !== undefined && inner_d.dim_1_num !== null ? inner_d.dim_1_num : inner_d.dim_1;
			var startDate = parseDateValue(startDateInput);
			var durationDays = +inner_d.meas_0;
			var endDate = addDays(startDate, durationDays);
